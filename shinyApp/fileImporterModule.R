...
fileImporterFile <- function(input, output, session, fi, fileOptions ) {
           ...
           d <- tryCatch({
               readxl::read_excel(userFile()$datapath,
                 sheet = sheet, col_names = col_names ) 
                  ...
                  flog.warn(str_c("loading excel with sheet:", w))
               }, error = function(e) {
                  flog.error(str_c("loading excel with sheet:", e))
  ...
  dataForExcel <- reactive({
    ...
    trans <- FALSE
    if("samplesAreRows" %in% names(input)) {
      flog.info(str_c("loading dataForExcel()"))
      trans <- input$samplesAreRows
      if(trans != TRUE) {
        d <- t(d)
      }
      flog.info(str_c("2 loading dataForExcel()"))
      d <- tryCatch({
         applyRowColumnFilters(d) 
      }, warning = function(w) {
         flog.warn(str_c("WARNING: ", w))
         browser()
         d
     }, error = function(err) { 
